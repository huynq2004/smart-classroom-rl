...
from algorithms.ar_q import ARQAgent
...
def main():
    ...
    parser.add_argument("--config", type=str, default="experiments/configs/arq.json")
    ...
    agent = ARQAgent(state_dim, action_dims, config={
    ...
    # Logging
    log_csv = os.path.join(out_dir, "logs", "arq_training_log.csv")
    ...
    for ep in range(episodes):
        ...
        print(f"[AR-Q] Ep {ep:04d} | R={ep_reward:.2f} | eps={agent.eps:.3f} | Eval={eval_mean:.2f}±{eval_std:.2f}")
        with open(log_csv, "a", newline="") as f:
            csv.writer(f).writerow([ep, ep_reward, agent.eps, eval_mean, eval_std])

        if save_every and (ep % save_every == 0 or ep == episodes - 1):
            torch.save(agent.q_net.state_dict(), os.path.join(out_dir, "models", f"arq_checkpoint_{ep}.pt"))

        if ep_reward > best_reward:
            best_reward = ep_reward
            torch.save(agent.q_net.state_dict(), os.path.join(out_dir, "models", "arq_best.pt"))

    torch.save(agent.q_net.state_dict(), os.path.join(out_dir, "models", "arq_latest.pt"))

    # plot
    fig_path = os.path.join(out_dir, "figures", "arq_reward_curve.png")
    plt.plot(rewards)
    plt.xlabel("Episode")
    plt.ylabel("Total Reward")
    plt.title("AR-Q Training Curve")
